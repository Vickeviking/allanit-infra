  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-gray-600">
          Hantera kundinformation och relaterade ordrar
        </p>
      </div>
    ...
    >
      <template #filters>
        <div class="flex items-center space-x-4">
          <div class="text-sm text-gray-600">
            {{ filteredCustomers.length }} av
            {{ customersStore.items.length }} kunder
          </div>
        </div>
      </template>
      ...
      <template #cell-contact="{ item }">
        <div>
          <p v-if="item.email" class="text-sm text-gray-900">
            {{ item.email }}
          </p>
          <p v-if="item.phone" class="text-xs text-gray-500">
            {{ item.phone }}
          </p>
        </div>
      </template>
      ...
      <template #header-actions>
        ...
        >
          <span class="text-sm text-gray-600"
            >{{ selectedCustomers.length }} valda</span
          >
          <button
            @click="bulkExport"
    ...
    <div v-if="selectedCustomer" class="fixed inset-0 z-50 overflow-hidden">
      <div
            ...
            <div class="space-y-4">
              <div>
                ...
                <div class="space-y-2">
                  <div class="flex items-center space-x-2">
                    ...
                    <span class="text-sm">{{
                      selectedCustomer.email || "Ej angiven"
                    }}</span>
                  </div>
                  <div class="flex items-center space-x-2">
                    <span class="text-sm text-gray-600">Telefon:</span>
                    <span class="text-sm">{{
                      selectedCustomer.phone || "Ej angiven"
                    }}</span>
                  </div>
                  <div class="flex items-center space-x-2">
                    <span class="text-sm text-gray-600">Org.nr:</span>
                    <span class="text-sm">{{
                      selectedCustomer.org_no || "Ej angivet"
                    }}</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div
                    class="text-center p-3 bg-white border border-gray-200 rounded"
                  >
                    <p class="text-lg font-semibold text-gray-900">
                      {{ getCustomerOrderCount(selectedCustomer.id) }}
                    </p>
                  ...
                  >
                    <p class="text-lg font-semibold text-gray-900">
                      {{ getCustomerTotalAmount(selectedCustomer.id) }} SEK
                    </p>
    ...
    customers = customers.filter((c) => {
      ...
      switch (filters.value.activity) {
        ...
        case "inactive":
          return orderCount === 0;
        case "new":
          return isNewCustomer(c.created_at);
        default:
          return true;
      }
    });
