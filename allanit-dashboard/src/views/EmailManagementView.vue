  ...
  <div class="max-w-screen-xl mx-auto px-4">
          ...
          <div v-else class="bg-white rounded-lg shadow-sm border overflow-hidden">
            <table class="w-full text-sm">
              <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th class="px-4 py-3 text-left font-medium text-gray-700">Namn</th>
                  <th class="px-4 py-3 text-left font-medium text-gray-700">Taggar</th>
                  <th class="px-4 py-3 text-left font-medium text-gray-700">Senast ändrad</th>
                  <th class="px-4 py-3 text-left font-medium text-gray-700">Åtgärder</th>
                </tr>
              </thead>
                ...
                >
                  <td class="px-4 py-3">
                    <div>
                      ...
                      <div class="text-xs text-gray-500 truncate max-w-xs">{{ template.subject }}</div>
                    </div>
                  </td>
                  <td class="px-4 py-3">
    ...
    <div v-if="activeTab === 'send'" class="space-y-6">
      <div class="bg-white rounded-lg shadow-sm border p-6">
        <h3 class="text-lg font-medium text-gray-900 mb-4">Skicka e-postutskick</h3>
        
        <div class="space-y-6">
          <!-- Template Selection -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Välj mall</label>
            <select
          ...
          <!-- Target Groups -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Målgrupper</label>
            <div class="space-y-2">
              <div
          ...
          <!-- Custom Recipients -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Anpassade mottagare</label>
            <textarea
          ...
          <div v-if="selectedTemplate">
            ...
            <div class="border rounded-md p-4 bg-gray-50">
              <h4 class="font-medium text-gray-900 mb-2">{{ selectedTemplate.subject }}</h4>
              <div v-html="selectedTemplate.html" class="text-sm text-gray-700"></div>
            </div>
          </div>
    ...
    <div v-if="activeTab === 'utskick'" class="space-y-6">
      <div class="bg-white rounded-lg shadow-sm border">
        <div class="px-6 py-4 border-b border-gray-200">
          <h3 class="text-lg font-medium text-gray-900">Skickade e-postmeddelanden</h3>
          <p class="text-sm text-gray-600 mt-1">Visa alla skickade e-postutskick</p>
        </div>
    ...
    <div v-if="activeTab === 'history'" class="space-y-6">
      <div class="bg-white rounded-lg shadow-sm border">
        <div class="px-6 py-4 border-b border-gray-200">
          <h3 class="text-lg font-medium text-gray-900">Utskickshistorik</h3>
        </div>
      ...
      <aside class="col-span-12 xl:col-span-5 xl:sticky xl:top-4">
        <div class="space-y-4">
          <!-- Template Preview -->
          <div v-if="selectedTemplate" class="bg-white rounded-lg shadow-sm border p-4">
            <h3 class="text-lg font-medium text-gray-900 mb-3">Förhandsvisning</h3>
          ...
          <div class="bg-white rounded-lg shadow-sm border p-4">
            <h3 class="text-lg font-medium text-gray-900 mb-3">Senaste utskick</h3>
            <div class="space-y-2">
              <div
              ...
              >
                <div>
                  <div class="text-sm font-medium text-gray-900">{{ send.campaign_name }}</div>
                  <div class="text-xs text-gray-500">{{ formatDate(send.sent_at) }}</div>
                </div>
                <div class="text-right">
                  <div class="text-sm text-gray-900">{{ send.delivered }}/{{ send.to_count }}</div>
                  <div class="text-xs text-gray-500">{{ Math.round((send.opened / send.delivered) * 100) || 0 }}% öppnat</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-lg shadow-sm border p-4">
            ...
            <div class="space-y-3">
              <div class="flex justify-between">
                ...
                <span class="text-sm font-medium text-gray-900">{{ overallStats.openRate }}%</span>
              </div>
              <div class="flex justify-between">
                <span class="text-sm text-gray-600">Klickgrad</span>
                <span class="text-sm font-medium text-gray-900">{{ overallStats.clickRate }}%</span>
              </div>
              <div class="flex justify-between">
                <span class="text-sm text-gray-600">Studsgrad</span>
                <span class="text-sm font-medium text-gray-900">{{ overallStats.bounceRate }}%</span>
              </div>
              <div class="flex justify-between">
                <span class="text-sm text-gray-600">Totalt skickat</span>
                <span class="text-sm font-medium text-gray-900">{{ overallStats.totalSent }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="h-full flex flex-col">
          ...
          <div class="flex-1 overflow-hidden flex">
              ...
              <div class="space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                  />
                  <p class="text-xs text-gray-500 mt-1">Använd taggar för att kategorisera mallen</p>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows="15"
                  ></textarea>
                  <p class="text-xs text-gray-500 mt-1">Använd HTML för att formatera e-postmeddelandet</p>
                </div>
              </div>
            </div>
            ...
            <div class="w-1/2 p-6 bg-gray-50">
              <div class="h-full flex flex-col">
                <h4 class="text-lg font-medium text-gray-900 mb-4">Live-förhandsvisning</h4>
                <div class="flex-1 border border-gray-300 rounded-lg bg-white overflow-y-auto">
                  <div class="p-6">
                    <div v-if="templateForm.subject" class="mb-4">
                      <h2 class="text-lg font-semibold text-gray-900">{{ templateForm.subject }}</h2>
                    </div>
                    <div v-if="templateForm.html" v-html="templateForm.html" class="text-gray-700"></div>
...
const canSendEmail = computed(() => {
  return emailForm.value.templateId && 
         (emailForm.value.segmentIds.length > 0 || emailForm.value.customRecipients.trim())
})
...
const overallStats = computed(() => {
  const totalSent = sentEmails.value.reduce((sum, send) => sum + send.to_count, 0)
  const totalDelivered = sentEmails.value.reduce((sum, send) => sum + send.delivered, 0)
  const totalOpened = sentEmails.value.reduce((sum, send) => sum + send.opened, 0)
  const totalFailed = sentEmails.value.reduce((sum, send) => sum + (send.to_count - send.delivered), 0)
  
  return {
    totalSent,
    openRate: totalDelivered > 0 ? Math.round((totalOpened / totalDelivered) * 100) : 0,
    clickRate: totalDelivered > 0 ? Math.round((totalOpened * 0.3 / totalDelivered) * 100) : 0, // Mock click rate
...
function saveTemplate() {
  const template = {
    ...
    tags: templateForm.value.tagsString.split(',').map(t => t.trim()).filter(t => t),
...
function sendEmail() {
  ...
  const template = templates.value.find(t => t.id === emailForm.value.templateId)
  if (!template) return
  ...
  const totalRecipients = emailForm.value.segmentIds.length + customEmails.length
