    ...
    pub fn spawn(shared: Arc<SharedResources>) -> JoinHandle<()> {
        tokio::spawn(async move {
            ...
            let mut ingestor_rx: Option<mpsc::UnboundedReceiver<EventPayload>> = {
                ...
                locked.take_rx(ChannelType::IngestorToDbWriter);
            };
            //TODO:  // Wiring: outgoing, nothing as of now, we can send ack later
            ...
            loop {
                select! {
                    ...
                    // valfri mpsc-ingång (kommando från DBWriter, t.ex. "synka nu")
                    maybe_msg = async {
                        if let Some(rx) = ingestor_rx.as_mut() { rx.recv().await } else { None }
                    } => {
                        if let Some(msg) = maybe_msg {
                            logger.debug("DBWriter", &format!("Received on wiring: {:?}", msg));
                        }
                    }
                }
            }
