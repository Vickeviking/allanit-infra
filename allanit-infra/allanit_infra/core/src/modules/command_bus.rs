    ...
    pub fn spawn(shared: Arc<SharedResources>) -> JoinHandle<()> {
        tokio::spawn(async move {
            ...
            let mut my_rx: Option<mpsc::UnboundedReceiver<EventPayload>> = {
                ...
                locked.take_rx(ChannelType::RocketToCommandBus);
            };
            ...
            let to_fetcher_tx: Option<mpsc::UnboundedSender<EventPayload>> = {
                ...
                locked.get_tx(ChannelType::CommandBusToFetcher).await
            };

            // Wiring: utgående TX till deadletter
            let to_dead_letter_tx: Option<mpsc::UnboundedSender<EventPayload>> = {
                let locked = wiring.lock().await; // read scope
                locked.get_tx(ChannelType::CommandBusToDeadLetter).await
            };
                ...
                select! {
                    ...
                    maybe_msg = async {
                        ...
                        if let Some(msg) = maybe_msg {
                            logger.debug("CommandBus", &format!("Received on wiring: {:?}", msg));
                        }
                    }
                }
            }
